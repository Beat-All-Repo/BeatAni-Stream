...
export function TitleBar() {
  ...
  const [title, setTitle] = useState("BeatAni");
  ...
  useEffect(() => {
    const updateTitle = () => {
        if (location.pathname === '/') setTitle('BeatAni');
        ...
        else setTitle(document.title?.replace(' | BeatAni', '') || 'BeatAni');
    };
    
    updateTitle();
    
    const observer = new MutationObserver(() => {
        setTitle(document.title?.replace(' | BeatAni', '') || 'BeatAni');
    });
